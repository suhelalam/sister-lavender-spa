...
export async function createVisit(adminDb, customer, input = {}) {
  ...
  const visit = {
    ...
    customerName: customer?.name || input.name || 'Guest',
    customerPhone: customer?.phone || input.phone || '',
    bookingId: input.bookingId || null,
    calendarEventId: input.calendarEventId || input.bookingId || null,
    source: input.source || (input.bookingId ? 'appointment' : 'walk-in'),
