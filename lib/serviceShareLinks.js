...
export const getServiceSelectionsFromCountQuery = (query = {}) =>
  Object.entries(query).flatMap(([key, rawCount]) => {
    if (!key.endsWith('-count')) return [];

    const selectionKey = key.slice(0, -'-count'.length);
    const countValue = Array.isArray(rawCount) ? rawCount[0] : rawCount;
    const count = Number.parseInt(countValue, 10);

    if (!selectionKey || !Number.isFinite(count) || count < 1) return [];

    // Keep malformed shared links from creating an excessive cart quantity.
    return [{ selectionKey, count: Math.min(count, 99) }];
  });

export const getServiceIdsFromCountQuery = (query = {}) =>
  getServiceSelectionsFromCountQuery(query).flatMap(({ selectionKey, count }) =>
    Array(count).fill(selectionKey)
  );
...
export const getServiceIdsFromRouter = (router) => {
  ...
  if (lastPart && !['services', 'booking'].includes(lastPart)) {
